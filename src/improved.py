...
def build_tfidf_item_features(df, max_features=100):
    df = df.copy()
    df['review_text'] = df['review_text'].fillna("").astype(str).str.strip()
    df = df[df['review_text'].str.len() > 0]

    if df.empty:
        raise ValueError("No valid review text for TF-IDF.")

    tfidf = TfidfVectorizer(max_features=max_features, stop_words='english')
    tfidf_matrix = tfidf.fit_transform(df['review_text'])

    tfidf_df = pd.DataFrame(tfidf_matrix.toarray(), columns=tfidf.get_feature_names_out())
    tfidf_df['product_id'] = df['product_id'].values
    tfidf_agg = tfidf_df.groupby('product_id').mean().reset_index()
    return tfidf_agg

def merge_features(tfidf_df, emoji_df):
    ...
    merged = pd.merge(tfidf_df, emoji_features, on='product_id', how='left').fillna(0)
    return merged
...
def run_model(df, item_feature_df):
    ...
    print("\n📊 Improved Hybrid Model (TF-IDF + Emoji Clusters):")
    for k, v in metrics.items():
        print(f"{k}: {v:.4f}")

def run_tfidf_pipeline(json_path: str):
    df = load_data(json_path, max_reviews=10000)
    tfidf_features = build_tfidf_item_features(df)
    emoji_features = cluster_emojis(df)
    combined_features = merge_features(tfidf_features, emoji_features)
